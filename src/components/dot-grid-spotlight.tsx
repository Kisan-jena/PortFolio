...
export function DotGridSpotlight({
  ...
  useEffect(() => {
    ...
    const defaultDotColor = isDark ? "rgba(255, 255, 255, 0.05)" : "rgba(0, 0, 0, 0.08)"
    const defaultActiveDotColor = isDark ? "rgba(255, 255, 255, 0.1)" : "rgba(0, 0, 0, 0.18)"

    const finalDotColor = dotColor || defaultDotColor
    const finalActiveDotColor = activeDotColor || defaultActiveDotColor
    ...
    const draw = () => {
      ...
      for (let x = offsetX; x <= width; x += spacing) {
        for (let y = offsetY; y <= height; y += spacing) {
          ...
          let currentColor = finalDotColor
          ...
          if (mouse.current.isActive && distance < interactionRadius) {
            ...
            currentColor = finalActiveDotColor
            currentAlpha =
